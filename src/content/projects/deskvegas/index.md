---
name: "DeskVegas"
description: "3D Las Vegas multiplayer board game"
...
endDate: "2025-06-23"
links:
  - label: "Release v1.3.1"
    url: "https://drive.google.com/drive/folders/1fyUR5zvjOu5nIPS-3dG_18SYs6vaPaC5?usp=sharing"
---

## Overview

DeskVegas는 **단 4일간의 개발로 완성한 3D 라스베가스 멀티플레이 보드게임**입니다. Unity 엔진과 [Mirror](https://github.com/MirrorNetworking/Mirror) 프레임워크를 기반으로 구축했으며, [Steamworks](https://github.com/rlabrecque/Steamworks.NET)를 이용해 Steam 플랫폼과 멀티플레이 환경을 통합했습니다.

친구들과 보드게임장에서 게임을 하다가 너무 재미있어서, 집에서도 같이 할 수 있게 직접 만들어 보면 어떨까 하는 생각에서 시작했습니다.

## 개발 일정

### 2025.06.16 | 네트워크 기반 핵심 코어 구현

:::note[2025.06.16 개발일지]
- 게임 로비, 방 참가, 가입, 씬 전환 완성
- 게임 매니저 추가
- 게임 플레이어 추가
- 메인 메뉴 UI 완성
- 인게임 UI 완성
...
![InGame UI](./inGameUI.webp)

---

### 2025.06.18 | 게임 로직 구현

:::note[2025.06.18 개발일지]
- 주사위 던지는 기능 추가
- 보드판에 돈 분배 기능 추가
- 보드판 시스템 추가
- 주사위 보드판에 배치 추가
...
:::note[2025.06.19 Release v1.1.0]
- `isReady` 확인 코드 추가
- MainMenu Lobby의 PlayerList에서 한글 이름이 깨지는 오류 해결
- InGame UI의 MaxRound가 4로 고정된 오류 해결
- 게임 종료 크래시 오류 해결
- 게임 자동 종료 시간 변경 (15초 → 10초)
- 게임 자동 종료 카운트다운 추가
- 앣타 망가짐님의 메인 메뉴 일러스트 추가
- MainMenu UI 전면 변경
- 턴 제한 시간 변경 (120초 → 60초)
:::
...
![MainMenu UI](./mainMenuUI.webp)

:::note[2025.06.23 Release v1.2.0]
- 이모지 시스템 추가
- 배경음악 시스템 추가
- 주사위 배치 시스템을 2줄에서 3줄로 수정
:::

:::note[2025.06.23 Release v1.3.0]
- 핑크색 주사위 하이라이트 오류 수정
- 디스코드 매니저 추가
:::
...
:::warning[문제 상황]
다른 씬으로 넘어갈 때 호스트가 클라이언트의 준비를 기다리지 않고 게임을 시작하는 문제가 발생했습니다. 예를 들어 메인 메뉴 씬에서 인게임 씬으로 넘어갈 때 재현되었습니다.
:::

:::tip[클라이언트가 호스트에게 준비 상태 전달]
...
인게임 씬으로 이동하면 호스트는 일정 시간 동안 `NetworkServer.connections.Count - 1`만큼, 즉 호스트를 제외한 클라이언트들의 응답을 기다립니다. 모든 응답을 받으면 `GameEvents.Network.OnSceneLoaded?.Invoke();`를 호출하여 게임을 시작하도록 수정했습니다.
...
:::warning[문제 상황]
ClientRpc 메시지가 SyncVar 업데이트보다 먼저 클라이언트에 도착하여 클라이언트가 이전 상태 값을 참조하는 문제가 발생했습니다.

예를 들어 턴이 Player 1에서 Player 2로 넘어갈 때 다음 순서로 문제가 재현되었습니다.
